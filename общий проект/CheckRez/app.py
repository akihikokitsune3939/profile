...
def get_usage_status() -> tuple[int, bool]:
    actor_key = get_actor_key()
    day = date.today().isoformat()
    con = sqlite3.connect(DB_PATH)
    row = con.execute(
        "SELECT checks_count, is_paid FROM usage_limits WHERE actor_key = ? AND day = ?",
        (actor_key, day),
    ).fetchone()
    con.close()
    if not row:
...
def add_usage_check() -> None:
    actor_key = get_actor_key()
    day = date.today().isoformat()
    con = sqlite3.connect(DB_PATH)
    con.execute(
        """
        INSERT INTO usage_limits(actor_key, day, checks_count, is_paid)
        VALUES (?, ?, 1, 0)
        ON CONFLICT(actor_key, day) DO UPDATE SET checks_count = checks_count + 1
        """,
        (actor_key, day),
    )
    con.commit()
...
def unlock_unlimited_for_today() -> None:
    actor_key = get_actor_key()
    day = date.today().isoformat()
    con = sqlite3.connect(DB_PATH)
    con.execute(
        ...
        INSERT INTO usage_limits(actor_key, day, checks_count, is_paid)
        VALUES (?, ?, 0, 1)
        ON CONFLICT(actor_key, day) DO UPDATE SET is_paid = 1
        """,
        (actor_key, day),
    )
    con.commit()
...
def detect_resume_fraud_flags(text: str) -> tuple[list[str], list[str], int, list[str], int]:
    truths: list[str] = []
    doubts: list[str] = []
    explain: list[str] = []
    penalty = 0
    ...
    risky_conditions = ["только telegram", "только предоплата", "быстрые деньги", "срочные проекты"]

    claim_hits = [x for x in extreme_claims if x in t]
    role_hits = [x for x in universal_roles if x in t]
    cond_hits = [x for x in risky_conditions if x in t]

    if claim_hits:
        add = 8 * min(3, len(claim_hits))
        penalty += add
        severe_hits += len(claim_hits)
        doubts.append("Обнаружены нереалистичные или маркетингово-агрессивные заявления.")
        explain.append(f"+{add}: нереалистичные обещания/достижения")

    if role_hits and len(role_hits) >= 2:
        add = 10
        penalty += add
        severe_hits += 1
        doubts.append("Слишком широкий и противоречивый профиль ролей в одном резюме.")
        explain.append(f"+{add}: противоречивый профиль ролей")

    if cond_hits:
        add = 7 * len(cond_hits)
        penalty += add
        severe_hits += len(cond_hits)
        doubts.append("Подозрительные условия сотрудничества (предоплата/только мессенджер).")
        explain.append(f"+{add}: рискованные условия коммуникации/оплаты")

    # Аномально большое количество проектов за короткий срок.
    if re.search(r"\b[5-9]\d{2,}\+\s*проект", t):
        penalty += 10
        severe_hits += 1
        doubts.append("Указан аномальный объем выполненных проектов за короткий период.")
        explain.append("+10: аномально высокий объем проектов")

    if not (claim_hits or role_hits or cond_hits):
        truths.append("Критических антифрод-маркеров в резюме не обнаружено.")
        explain.append("-2: нет критических антифрод-маркеров")
        penalty -= 2

    return truths, doubts, penalty, explain, severe_hits
...
def analyze_resume(text: str, deep: bool = False):
    ...
    t_rf, d_rf, p_rf, e_rf, severe_hits = detect_resume_fraud_flags(text)
    truths.extend(t_rf)
    doubts.extend(d_rf)
    explain.extend(e_rf)
    score = max(1, min(100, score + p_rf))
    flags_count += len(d_rf)

    # Жесткий потолок для явно сомнительных резюме.
    if severe_hits >= 3:
        score = max(score, 70)  # риск
        doubts.append("Применен антифрод-порог из-за множества критических маркеров.")
        explain.append("+10: антифрод-порог по критическим маркерам")
    elif severe_hits >= 2:
        score = max(score, 62)
        explain.append("+6: повышенный риск по совокупности критических маркеров")
...
@app.route("/pay/unlock", methods=["POST"])
def pay_unlock_page():
    unlock_unlimited_for_today()
    target = request.form.get("target", "resume")
    if target == "vacancy":
        return redirect(url_for("vacancy_page"))
...
def pay_page():
    warning = ""
    target = request.values.get("target", "resume")
    if request.method == "POST":
        card_number = (request.form.get("card_number") or "").replace(" ", "")
        card_name = (request.form.get("card_name") or "").strip()
        exp = (request.form.get("exp_date") or "").strip()
        cvv = (request.form.get("cvv") or "").strip()
        plan = (request.form.get("plan") or "").strip()

        valid_card = card_number.isdigit() and 13 <= len(card_number) <= 19
        valid_name = len(card_name) >= 3
        valid_exp = bool(re.fullmatch(r"(0[1-9]|1[0-2])\/\d{2}", exp))
        valid_cvv = bool(re.fullmatch(r"\d{3,4}", cvv))
        valid_plan = plan in {"day", "month", "year"}

        if not (valid_card and valid_name and valid_exp and valid_cvv and valid_plan):
            warning = "Проверьте данные карты и тариф: номер/имя/срок/CVV должны быть заполнены корректно."
        else:
            # Демо-оплата: реальная эквайринг-интеграция не подключена.
            unlock_unlimited_for_today()
            if target == "vacancy":
                return redirect(url_for("vacancy_page"))
            return redirect(url_for("resume_page"))
    return render_template("pay.html", warning=warning, target=target, user=current_user())
...
def resume_page():
    context = {
        ...
        "daily_limit": DAILY_CHECK_LIMIT,
        "daily_used": 0,
        "is_paid": False,
        "paywall": False,
    }
    used, is_paid = get_usage_status()
    context["daily_used"] = used
    ...
    if request.method == "POST":
        ...
        if not is_paid and used >= DAILY_CHECK_LIMIT:
            context["paywall"] = True
            context["warnings"].append("Лимит 10 проверок в день исчерпан. Оплатите, чтобы продолжить без ограничений.")
            return render_template("resume.html", user=current_user(), **context)
